...
class ModelArguments:
    ...
    geometry_encoder_type: str = field(default="vggt")  # Type of geometry encoder
    ...
    feature_fusion_method: str = field(default="add")  # Method to fuse geometry and visual features ("add", "concat", "cross_attention", "gated", "weighted", "leiside_q_former")
    ...
    vision_mask_apply_prob: float = field(
        default=0.5,
        metadata={"help": "Probability to enable random vision token masking for a training sample."},
    )
    vision_mask_prob: float = field(
        default=0.8,
        metadata={"help": "Per-token mask probability when random vision token masking is enabled."},
    )
