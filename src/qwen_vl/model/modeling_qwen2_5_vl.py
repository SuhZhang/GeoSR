...
class Qwen2_5_VLForConditionalGenerationWithVGGT(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def _process_geometry_features(self, image_embeds, geometry_encoder_inputs):
        ...
        for bn in range(batch_size):
            if geometry_encoder_inputs[bn].shape[0] > 0:
                ...
                features = self.geometry_encoder.encode(geometry_encoder_inputs[bn]).to(
    ...
    def _should_apply_gated_random_mask(self) -> bool:
        """Enable random vision masking only for gated geometry fusion."""
        return (
            bool(getattr(self.config, "use_geometry_encoder", False))
            and getattr(self.config, "feature_fusion_method", "add") == "gated"
        )

    def _get_vision_keep_prob(self, device: torch.device) -> float:
        ...
        vision_keep_prob = 1.0 if not self.training else (1.0 - vision_mask_prob)
        if self.training:
            # With prob (1 - vision_mask_apply_prob), disable masking by keeping all tokens.
            if torch.rand(1, device=device) >= vision_mask_apply_prob:
                vision_keep_prob = 1.0
        return vision_keep_prob

    def _mask_vision_embeds(self, embeds: Optional[torch.Tensor], vision_keep_prob: float) -> Optional[torch.Tensor]:
        if embeds is None:
            return embeds
        if (not self.training) or vision_keep_prob >= 1.0:
            return embeds
        num_tokens = embeds.shape[0]
        if num_tokens == 0:
            return embeds
        keep = torch.rand(num_tokens, device=embeds.device) < vision_keep_prob
        if not torch.any(keep):
            keep[torch.randint(0, num_tokens, (1,), device=embeds.device)] = True
    ...
    def forward(
        ...
        **kwargs,
        ...
        if inputs_embeds is None:
            ...
            vision_keep_prob = 1.0
            apply_gated_random_mask = self._should_apply_gated_random_mask()
            if apply_gated_random_mask:
                vision_keep_prob = self._get_vision_keep_prob(inputs_embeds.device)
            if pixel_values is not None:
                pixel_values = pixel_values.type(self.visual.dtype)
                image_embeds = self.visual(pixel_values, grid_thw=image_grid_thw)
                gated_prefusion_mask_applied = False
                ...
                if getattr(self.config, 'use_geometry_encoder', False) and geometry_encoder_inputs is not None:
                    if apply_gated_random_mask:
                        # For gated fusion, mask 2D visual values before fusion to push reliance on 3D tokens.
                        image_embeds = self._mask_vision_embeds(image_embeds, vision_keep_prob)
                        gated_prefusion_mask_applied = True
                ...
                if not gated_prefusion_mask_applied:
                    image_embeds = self._mask_vision_embeds(image_embeds, vision_keep_prob)
            ...
            if pixel_values_videos is not None:
                ...
                video_embeds = self._mask_vision_embeds(video_embeds, vision_keep_prob)
